...
| Jul 1 | Scope Engine lead-capture fix (Claude Code): diagnosed why `scope-engine.html` submissions weren't emailing. Backend (`formsubmit.co/ajax/`) is healthy — verified via live test POSTs that delivered. Three client-side defects fixed in the submit handler: (1) success was judged by `r.ok` (HTTP status) but formsubmit ALWAYS returns 200 — real result is JSON body `success`; now branches on `String(data.success)==='true'`, so failures no longer show a false "emailed" message or fire phantom `generate_lead`; (2) `buildLeadPayload()` moved INSIDE the try/catch (was outside — a bad `state` threw uncaught and froze the UI on "Sending…"); (3) added 15s AbortController timeout + shared `fail()` helper that re-enables the button and preserves inputs on every error path. Also: `_template` `table`→`basic` (deliverability, matches §4), and corrected copy that claimed the PDF is emailed — the `/ajax/` endpoint silently STRIPS attachments (verified: test email arrived `hasAttachments:false`), so ACG gets scope data only and the visitor gets the PDF via client download. To email ACG the PDF automatically, stand up the Cloudflare Worker in SCOPE-ENGINE-README.md §4. | Real GC leads from the scope engine were never reaching ACG (inbox audit: zero `New scope-engine lead` emails ever), while visitors saw a success message — silent lead loss on a conversion-critical tool. |
...
- **Scope Engine Cloudflare Worker NOT deployed** — `workers/scope-engine-api/` (added Jul 1) emails ACG + the visitor the branded PDF via Resend (formsubmit `/ajax` can't carry attachments). Needs `wrangler deploy` + a Resend API key + `acglass.com` verified in Resend. Until deployed, `scope-engine.html` stays on the formsubmit endpoint (data-only to ACG, PDF via client download — working as of the Jul 1 fix). To go live: deploy, then repoint `ACG_CONFIG.endpoint` in scope-engine.html at the Worker URL (one line) and update the visitor success copy. See `workers/scope-engine-api/README.md`.
