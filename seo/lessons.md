...
- City roots that were indexable and canonicalized to a wave-2 noindex `storefront-glazier-*` URL now canonical to `/florida-commercial-glazing/` (indexable hub), never to a noindex template. The 8 keepers and the 324 noindex templates were not redesigned. `/west-palm-beach/`, `/naples/`, and `/tampa/` were left for Wave-4 (#94). Satellite roots Miami/Orlando/Fort Lauderdale/Fort Myers/Sarasota still canonical to their indexable keepers.
- Homepage WebSite JSON-LD description no longer says “Florida & Tennessee commercial glazing contractor.” Title/meta/og/H1 were not edited.
- `/es-windows.html` no longer links `eswindows.com/product/*` (live 404 / Cloudflare-blocked). CTAs go to `https://eswindows.com`. `/products/eswindows/` was not created.
- `/acg-nashville-office-opening/` is a Gone stub; `cloudflare-410-worker.js` lists the path for an edge 410. `/tennessee-commercial-glazing/` title/og dropped the orphan “Q3”. `/nashville/` no longer says “moving into Nashville.”
